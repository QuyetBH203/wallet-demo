...
export default function CreateWalletPage() {
  ...
  const handleCreateWallet = async () => {
    ...
    try {
      ...
      alert(
        `IMPORTANT: Please save these 12 words in a safe place. This is the only way to recover your wallet:\n\n${generatedMnemonicPhrase}`
      )
      ...
      // encypt private key to keystore JSON with password
  ...
  if (walletCreated) {
    return (
      <div className='flex flex-col items-center justify-center h-full p-4 gap-4 text-center'>
        ...
        {mnemonicPhrase && (
          <div className='mb-4 p-3 bg-gray-100 border border-gray-300 rounded w-full max-w-md'>
            ...
            <p className='text-sm text-red-600 mb-2'>
              <strong>Store these words in a safe place. This is the ONLY way to recover your wallet.</strong>
            </p>
        ...
        <Button variant='outline' className='w-full max-w-xs mt-2' onClick={() => window.close()}>
          Close Tab
        </Button>
      </div>
    )
    ...
    <div className='flex flex-col items-center justify-center h-full p-4 gap-4'>
      ...
      {mnemonicPhrase && !walletCreated && (
        <div className='mb-4 p-3 bg-yellow-100 border border-yellow-400 rounded w-full max-w-md'>
          ...
          <p className='text-sm text-red-600 mb-2'>
            <strong>
              IMPORTANT: Write down these words in the exact order and store them in a secure offline location. Anyone
              with these words can access your funds.
            </strong>
          </p>
          <p className='font-mono bg-gray-200 p-2 rounded break-words'>{mnemonicPhrase}</p>
          <p className='text-xs mt-2'>
            After saving your phrase, the wallet will be encrypted with your password and stored.
          </p>
        </div>
      )}
      ...
      {!mnemonicPhrase && (
        <>
          <p className='text-sm text-gray-600 mb-3 text-center max-w-xs'>
            Create a strong password to secure your new wallet. This password will be used to encrypt your private keys.
          </p>
      ...
      >
        {mnemonicPhrase
          ...
          : 'Generate Recovery Phrase & Create Wallet'}
      </Button>
      {mnemonicPhrase && !walletCreated && !error && (
        <p className='text-xs mt-2'>Wallet is being generated, encrypted and saved...</p>
      )}
    </div>
  )
}
